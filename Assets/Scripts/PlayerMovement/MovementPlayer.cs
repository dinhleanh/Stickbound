...
public class MovementPlayer : MonoBehaviour
{
    ...
    private void SetFacingDirection(Vector2 _moveVector)
    {
        ...
        {
            if (moveTo.x > rb.transform.position.x + 1) // +1 fixt wenn man grade nach oben grapplet
            {
            ...
            else if (moveTo.x < rb.transform.position.x - 1)
            {
    ...
    void MouseDuration()
    {
        ...
        if (isMousePressed)
        {
            ...
            xVelocityAfterGrappleInfluencedByMouseDurationClamped = Mathf.Clamp(xVelocityAfterGrappleInfluencedByMouseDuration, 0, 0.5f);
    ...
    Vector2 GetMousePositionAndReturnDirection()
    {
        ...
        Vector2 direction = (mousePos - rb.position).normalized; // Im tutorial (Vector2)transform.position statt rb.position
        return direction;
    }
    ...
    public void OnGrapple()
    {
        ...
        if (points.Count > 0 && didGrappleHit)
        {
            ...
            rb.MovePosition(Vector2.MoveTowards(rb.position, moveTo, moveSpeedFromGrapple));
        ...
        if (Input.GetMouseButtonUp(0) && IsGrappling)
        {
            ...
            if (moveVector.x == 0f && didGrappleHit)
            {
                ...
                {
                    rb.velocity = new Vector2(20 * xVelocityAfterGrappleInfluencedByMouseDurationClamped, 0.2f);
